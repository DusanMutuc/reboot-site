...
export default function AdminPage() {
  ...
  if (loading) {
    return (
      <Container maxWidth="lg" sx={{ py: 6, display: 'grid', placeItems: 'center', minHeight: '60vh' }}>
        <CircularProgress />
        <Typography sx={{ mt: 2 }} variant="body2">Checking access…</Typography>
      </Container>
    );
  }
  ...
  if (!user) {
    return (
      <Container maxWidth="sm" sx={{ py: 6 }}>
        <Alert severity="info" sx={{ mb: 2 }}>You must be logged in.</Alert>
  ...
  if (!isAdmin) {
    return (
      <Container maxWidth="sm" sx={{ py: 6 }}>
        <Alert severity="warning" sx={{ mb: 2 }}>Access denied — Admin privileges required.</Alert>
        <Button variant="outlined" onClick={() => router.push('/dashboard')}>Go to Dashboard</Button>
      </Container>
    );
  }
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Paper elevation={1} sx={{ borderRadius: 2 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} variant="scrollable"
              textColor="primary" indicatorColor="primary" sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <Tab label="Add User" />
