...
export async function GET(request: NextRequest) {
  ...
  const cycles = (cyclesResult.data ?? []).map((cycle) => ({
    ...
    members: enrollmentRows
      ...
      .map((row) => ({ ...row, name: people.find((person) => person.id === row.user_id)?.name ?? 'Unnamed user' })),
  }));
...
export async function POST(request: NextRequest) {
  ...
  if (action === 'create-cycle') {
    ...
    if (!name || !start.isValid || !IANAZone.isValidZone(timezone)) {
      return NextResponse.json({ error: 'Name, start date, and a valid timezone are required' }, { status: 400 });
    }
...
export async function PATCH(request: NextRequest) {
  ...
  if (action === 'configure-cycle') {
    ...
    if (!cycleId || !name || !DateTime.fromISO(startsOn).isValid || !IANAZone.isValidZone(timezone)) {
      return NextResponse.json({ error: 'Cycle, name, start date, and timezone are required' }, { status: 400 });
    }
    ...
    const { error } = await supa.rpc('configure_ninety_day_cycle', {
      p_cycle_id: cycleId,
      p_name: name,
  ...
  if (action === 'update-meeting') {
    const meetingId = positiveInteger(body.meeting_id);
    const title = stringValue(body.title) || 'Weekly group call';
    const startsAt = stringValue(body.starts_at);
    const endsAt = stringValue(body.ends_at);
    const joinUrl = stringValue(body.join_url);
    if (!meetingId || !DateTime.fromISO(startsAt).isValid || (endsAt && !DateTime.fromISO(endsAt).isValid)) {
      return NextResponse.json({ error: 'Meeting and a valid start are required' }, { status: 400 });
    }
    if (!validHttpUrl(joinUrl)) {
      return NextResponse.json({ error: 'Meeting URL must use http or https' }, { status: 400 });
    }
    const { error } = await supa
      .from('ninety_day_cycle_meetings')
      .update({ title, starts_at: startsAt, ends_at: endsAt || null, join_url: joinUrl || null })
      .eq('id', meetingId);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    return NextResponse.json({ ok: true });
  }
