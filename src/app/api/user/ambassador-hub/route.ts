...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (profileError) {
      ...
      return NextResponse.json({ error: 'Could not load ambassador link.' }, { status: 500 });
    }

    const cachedContactId = readString(profile?.ghl_user_id);
    if (cachedContactId) {
      const firstName =
        getProfileFirstName(profile) ?? extractUserMetadataFirstName(guard.user.user_metadata);
    ...
    const email = guard.user.email?.trim().toLowerCase();
    if (!email) {
      return NextResponse.json({ error: 'No email found for authenticated user.' }, { status: 400 });
    }

    if (!GHL.BASE || !GHL.TOKEN || !GHL.VERSION) {
      return NextResponse.json({ error: 'GHL contact lookup is not configured.' }, { status: 500 });
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Could not load ambassador link.' }, { status: 502 });
    }
    ...
    if (!contact) {
      ...
      return NextResponse.json({ error: 'No GHL contact found for this user.' }, { status: 404 });
    }
    ...
    if (!contactId) {
      ...
      return NextResponse.json({ error: 'GHL contact ID is not available for this user.' }, { status: 404 });
    }
    ...
    const firstName =
      ...
      extractUserMetadataFirstName(guard.user.user_metadata) ??
      '';

    return NextResponse.json({ url: buildHubUrl(firstName, contactId) });
  } catch (error) {
