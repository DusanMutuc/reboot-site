      ...
      <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1}>
          ...
          <Tooltip title="Back to grid view">
            <IconButton size="medium" component={Link} href={BASE_PATH}>
...
export default function AssistantLibraryLayout({ children }: { children: React.ReactNode }) {
  const router = useRouter();
  const selectedSegment = useSelectedLayoutSegment(); // null on /assistant-library, '<slug>' on /assistant-library/[slug]
  ...
  useEffect(() => {
    ...
    if (lesson && lesson.children?.length) {
      ...
      if (firstChapter && firstChapter !== selectedSlug) {
        router.push(`${BASE_PATH}/${firstChapter}`);
      }
    }
  ...
  const openItem = useCallback(
    (slug: string) => {
      ...
      if (lesson) {
        ...
        if (selectedSlug !== target) router.push(`${BASE_PATH}/${target}`);
        return;
      }
      if (selectedSlug !== slug) router.push(`${BASE_PATH}/${slug}`);
    },
    [items, selectedSlug, router]
  );

  // If we're on /assistant-library (no slug), show children full-width; otherwise show sidebar + content.
