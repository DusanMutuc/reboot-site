...
export default function TrainingAssignmentPanel(props: TrainingAssignmentPanelProps) {
  // A cycle change must discard editor state and any pending response for the old cycle.
  return <TrainingAssignmentEditor key={`${props.userId}:${props.coachingNoteId}`} {...props} />;
}

function TrainingAssignmentEditor({
  userId,
  coachingNoteId,
  ...
  async function saveAssignment() {
    if (!selectedCourse || loading || loadFailed || saving) return;
  ...
  async function removeAssignment() {
    if (!assignment || loading || loadFailed || saving) return;
          ...
          <>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.25}>
              <Button
                ...
                disabled={loading || loadFailed || !selectedCourse || saving || (Boolean(assignment) && !hasChanges)}
